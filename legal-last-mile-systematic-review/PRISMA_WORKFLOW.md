...
**750 have been screened (347 include / 403 exclude)**, with the researcher
...
(full extraction) is likewise live and now fully caught up: **all 347
current full-text includes (S001–S347) are fully extracted** into
`extraction_database.csv` against `CODEBOOK.md`'s full 92-field schema.
Phase 9 (risk of bias) is
intentionally not yet applied to any of the 347 — `risk_of_bias_tool` is
...
against all 347 extracted studies, both the mechanically-derivable fields
(via
