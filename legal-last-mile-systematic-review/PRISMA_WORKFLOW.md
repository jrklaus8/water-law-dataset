...
**757 have been screened (350 include / 407 exclude)**, with the researcher
...
(full extraction) is likewise live and now fully caught up: **all 350
current full-text includes (S001–S351, S227 a documented gap) are fully extracted** into
`extraction_database.csv` against `CODEBOOK.md`'s full 92-field schema.
Phase 9 (risk of bias) is
intentionally not yet applied to any of the 350 — `risk_of_bias_tool` is
...
against all 350 extracted studies, both the mechanically-derivable fields
(via
