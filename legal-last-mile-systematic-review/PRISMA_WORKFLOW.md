...
**724 have been screened (329 include / 395 exclude)**, with the researcher
...
(full extraction) is likewise live and now fully caught up: **all 329
current full-text includes (S001–S329) are fully extracted** into
`extraction_database.csv` against `CODEBOOK.md`'s full 92-field schema.
Phase 9 (risk of bias) is
intentionally not yet applied to any of the 329 — `risk_of_bias_tool` is
...
against all 329 extracted studies, both the mechanically-derivable fields
(via
