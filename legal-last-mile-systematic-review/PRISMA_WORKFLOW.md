...
**193 have been screened (92 include / 101 exclude)**, with the researcher
...
(full extraction) is likewise live and now fully caught up: **all 92
current full-text includes (S001–S092) are fully extracted** into
`extraction_database.csv` against `CODEBOOK.md`'s full 92-field schema.
Phase 9 (risk of bias) is
intentionally not yet applied to any of the 92 — `risk_of_bias_tool` is
...
against all 92 extracted studies, both the mechanically-derivable fields
(via
