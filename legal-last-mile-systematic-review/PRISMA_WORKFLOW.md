...
**781 have been screened (366 include / 415 exclude)**, with the researcher
...
(full extraction) is likewise live and now fully caught up: **all 366
current full-text includes (S001–S367, S227 a documented gap) are fully extracted** into
`extraction_database.csv` against `CODEBOOK.md`'s full 92-field schema.
Phase 9 (risk of bias) is
intentionally not yet applied to the great majority of the 366 (a first
12-study partial pilot batch was appraised 2026-09-16, see the Phase 9 row
above) — `risk_of_bias_tool` is
identified per study but `risk_of_bias_rating` is deliberately left blank
pending the official version of each appraisal instrument, per
...
against all 366 extracted studies, both the mechanically-derivable fields
(via
