...
Two files, generated 2026-09-10 and refreshed 2026-09-11 (once `SEARCH_026`,
the last of 18 planned Scopus batches, was screened) from
`screening_database.csv` after Claude's first-pass screening (`reviewer_1`,
tag `Claude-AI-1stpass-2026-09-10`) — see `CHANGELOG.md` and
`ai_first_pass_rationale.csv` for the full method.

## `reviewer_2_queue.csv` — the actual work

Every record `reviewer_1` marked `include` (1,509) or `unsure` (189) — 1,698
...
title/abstract stage, and screening only these 1,698 does not by itself
...
A **random** sample of 100 of the 3,747 records `reviewer_1` excluded
(~2.7%), fixed seed `20260911` for reproducibility (anyone can regenerate
...
single full pass over 3,747 records has no other way to surface cheaply.
...
over all 3,747), not to fix only the 100 sampled rows.
